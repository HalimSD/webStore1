...
namespace WebApp1.Controllers.Admin
{
    ...
    public class CategoryList : Controller
    {
        ...
        {
            Category parent = (from p in context.Category where p.Id == id select p).FirstOrDefault();
        ...
        public JsonResult GetData(int pageIndex = 1)
        {
            ...
            PaginationViewModel<CategoryListViewModel> json =
                new CategoryListViewModelHelper().ConvertToCategoryViewModel(context, categoryPage);

            return new JsonResult(json);
        }

        [Route("GetDataFiltered")]
        public JsonResult GetDataFiltered(string id = "", string name = "", string productCount = "",
            string attributeCount = "", int pageIndex = 1)
        {
